...
public class HomePageController {
    ...
    public String homepageSetup(Model model, HttpServletRequest request) {
        ...
        for (Post p : postList) {
            ...
            Optional<User> optionalUser = userRepository.findById(p.getUserId());
    ...
    public Model setupSinglePostPage(int id, Model model, HttpServletRequest request) throws Exception {
        ...
        Post post = optionalPost.orElseThrow(() -> new EntityNotFoundException("Post not found"));
        post.setVoteCount(voteRepository.countVotesByPostId(post.getId()));

        Optional<User> optionalPostUser = userRepository.findById(post.getUserId());
        User postUser = optionalPostUser.orElseThrow(() -> new EntityNotFoundException("User not found"));
    ...
    public Model setupEditPostPage(int id, Model model, HttpServletRequest request) throws Exception {
        if (request.getSession(false) != null) {
            ...
            returnPost.setVoteCount(voteRepository.countVotesByPostId(returnPost.getUserId()));
